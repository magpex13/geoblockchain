...
use sc_consensus_pow::{ MiningHandle, PowAlgorithm };
...
use sc_consensus::import_queue::{ BoxBlockImport, BasicQueue, Verifier, BoxJustificationImport };
...
pub enum Error<B: BlockT> {
	#[display(fmt = "Header uses the wrong engine {:?}", _0)] WrongEngine([u8; 4]),
	#[display(fmt = "Header {:?} is unsealed", _0)] HeaderUnsealed(B::Hash),
	...
	TooFarInFuture,
	#[display(
		fmt = "Fetching best header failed using select chain: {:?}",
		_0
	)] BestHeaderSelectChain(ConsensusError),
	#[display(fmt = "Fetching best header failed: {:?}", _0)] BestHeader(sp_blockchain::Error),
	#[display(fmt = "Best header does not exist")]
	NoBestHeader,
	#[display(fmt = "Block proposing error: {:?}", _0)] BlockProposingError(String),
	#[display(fmt = "Fetch best hash failed via select chain: {:?}", _0)] BestHashSelectChain(
		ConsensusError,
	),
	#[display(fmt = "Error with block built on {:?}: {:?}", _0, _1)] BlockBuiltError(
		B::Hash,
		ConsensusError,
	),
	#[display(fmt = "Creating inherents failed: {}", _0)] CreateInherents(sp_inherents::Error),
	#[display(fmt = "Checking inherents failed: {}", _0)] CheckInherents(String),
...
impl<Difficulty> PowAux<Difficulty> where Difficulty: Decode + Default {
...
impl<B: BlockT, I: Clone, C, S: Clone, Algorithm: Clone, CIDP: Clone> Clone
		...
		CIDP: CreateInherentDataProviders<B, ()> + Send + Sync
{
	...
	fn import_block(
		...
		if block.fork_choice.is_none() {
			block.fork_choice = Some(
				ForkChoiceStrategy::Custom(match aux.total_difficulty.cmp(&best_aux.total_difficulty) {
					Ordering::Less => false,
...
impl<B: BlockT, Algorithm> PowVerifier<B, Algorithm> {
	...
	fn check_header(
		...
		where Algorithm: PowAlgorithm<B>
	{
...
impl<B: BlockT, Algorithm> Verifier<B>
	for PowVerifier<B, Algorithm>
	where Algorithm: PowAlgorithm<B> + Send + Sync, Algorithm::Difficulty: 'static
{
	fn verify(
		&mut self,
		origin: BlockOrigin,
		...
		CIDP: CreateInherentDataProviders<Block, ()>,
{
	...
	let worker = Arc::new(
		Mutex::new(MiningWorker::<Block, Algorithm, C> {
			build: None,
	...
	loop {
		...
		let proposal = match
			proposer.propose(inherent_data, inherent_digest, build_time.clone(), RecordProof::No).await
		{
			Ok(x) => x,
			Err(err) => {
				warn!(
